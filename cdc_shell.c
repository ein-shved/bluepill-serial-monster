...
#define cdc_shell_msg_pin(pinn, fmt, ...) cdc_shell_msg("%6s [%d]: " fmt,      \
                    gpion_to_str(pinn), gpion_pin_get_free(pinn, 0), ## __VA_ARGS__)
...
#define cdc_shell_pin_extra_func_status(func, var, status_val, name, ...) do {\
    ...
    cdc_shell_pin_extra_func(func, var, \
        __device_config->gpio_config.pins[__pinn].status == status_val, name, ## __VA_ARGS__); \
} while (0)

#define cdc_shell_pin_extra_func_uart(func, var, port_num, ...) do {\
    cdc_shell_pin_extra_func(func, var, gpion_to_cdc(__pinn).port == (port_num) - 1, \
...
#define cdc_shell_pin_extra_func_free(func, var, ...) \
    cdc_shell_pin_extra_func_status(func, var, gpio_status_free, free, ## __VA_ARGS__)

#define cdc_shell_pin_extra_func_blocked(func, var, ...) \
    cdc_shell_pin_extra_func_status(func, var, gpio_status_blocked, blocked, ## __VA_ARGS__)

#define cdc_shell_pin_extra_func_occupied(func, var, ...) \
    cdc_shell_pin_extra_func_status(func, var, gpio_status_occupied, occupied, ## __VA_ARGS__)
...
#define cdc_shell_pin_check_extra_func(name, done_var, check_var, pin_var) do { \
    if (!done_var && !strcmp(check_var, #name)) { \
        (pin_var) = cdc_shell_gpion_pin_ref_ ## name; \
        done_var = 1; \
    } \
} while (0)

#define cdc_shell_pin_apply_extras(...) \
    cdc_shell_pin_declare_extras(cdc_shell_pin_declare_extra_func, ## __VA_ARGS__)

#define cdc_shell_pin_check_extras(...) \
    cdc_shell_pin_declare_extras(cdc_shell_pin_check_extra_func, ## __VA_ARGS__)
...
static void cdc_shell_cmd_gpio(int argc, char *argv[]) {
    ...
    {
        int found_pin = 0;
        cdc_shell_pin_check_extras(found_pin, argv[0], pin);
        if (!found_pin) {
